...
def ctof(x):
    return int(((float(x) * 9.0) / 5.0) + 32.0)
...
def weather_print_result(result, mode='summer'):
    try:
        ...
        heatindex = pywapi.heat_index(float(tempc),
                                      humidity,
                                      units='metric') or ctof(tempc)
        if mode == 'winter':
            printthis = "{0} Currently {1} {2}F/{3}C  " \
                        "Forecast: {4} (High {5}F/{6}C - Low {7}F/{8}C) " \
                        "[Wind: chill {9}F/{10}C speed {11}mph/{12}kmh]"
            printthis = printthis.format(title, text, ctof(tempc),
                                         tempc, forecast, ctof(high),
                                         high, ctof(low), low,
                                         ctof(windchillc), windchillc,
                                         kmhtomph(windspeedkm),
                                         int(float(windspeedkm)))
        if mode == 'summer':
            printthis = "{0} Currently {1} {2}F/{3}C " \
                        "Forecast: {4} (High {5}F/{6}C - Low {7}F/{8}C) " \
                        "[Humidity: {9}% - Heat Index: {10}F/{11}C - " \
                        "Wind: {12}mph/{13}kmh]"
            printthis = printthis.format(title, text, ctof(tempc),
                                         tempc, forecast, ctof(high),
                                         high, ctof(low), low,
